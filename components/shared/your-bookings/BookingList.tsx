...
/**
 * BookingList component displays a list of bookings with infinite scroll and pull-to-refresh
 */
export default function BookingList({ 
  ...
  const [state, setState] = useState<BookingListState>({
    items: bookingListFetch.items || [],
    page: INITIAL_PAGE,
    hasMore: bookingListFetch.items.length < bookingListFetch.total,
    loading: false,
    refreshing: false,
    error: bookingListFetch.error || null
  });
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      const response = await fetchBookingList({ page: state.page });
      ...
      setState(prev => ({
        ...prev,
        loading: false,
        error: 'Failed to load more booking history.'
      }));
    }
  ...
  const lastBookingRef = useInfiniteScroll({
    onLoadMore: loadMore,
